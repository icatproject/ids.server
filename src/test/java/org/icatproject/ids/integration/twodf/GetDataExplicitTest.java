...
public class GetDataExplicitTest extends BaseTest {
	...
	public void gettingDatafileDoesNotRestoreItsDatasetTest() throws Exception {
		...
		try (InputStream z = testingClient.getData(sessionId,
				new DataSelection().addDatafile(datafileIds.get(2)), Flag.NONE, null, 0, 404)) {
		...
		waitForIds();
		try (InputStream stream = testingClient.getData(sessionId,
				new DataSelection().addDatafile(datafileIds.get(3)), Flag.NONE, null, 0, 404)) {
			fail("Should have thrown an exception");
		} catch (DataNotOnlineException e) {
			// All is well
		}
	...
	public void gettingDatasetUsesCacheTest() throws Exception {
		...
		try (InputStream z = testingClient.getData(sessionId,
				new DataSelection().addDataset(datasetIds.get(0)), Flag.NONE, null, 0, 404)) {
			fail("Should have thrown an exception");
		} catch (DataNotOnlineException e) {
			// All is well
		}
	...
	public void gettingDatafileAndDatasetShouldNotRestoreBothDatasetsTest() throws Exception {
		...
		waitForIds();
		try (InputStream stream = testingClient.getData(sessionId,
				new DataSelection().addDatasets(datasetIds), Flag.NONE, null, 0, 404)) {
			fail("Should have thrown an exception");
		} catch (DataNotOnlineException e) {
			// All is well
		}
