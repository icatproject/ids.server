...
public class GetDataExplicitTest extends BaseTest {
    ...
    public void getSizes1() throws Exception {
        ...
        assertEquals(
            ...
            testingClient.getSize(
                sessionId,
                new DataSelection().addDatasets(datasetIds),
                200
            )
        );
        assertEquals(
        ...
        assertEquals(
            ...
            testingClient.getSize(
                sessionId,
                new DataSelection()
                    .addInvestigation(investigationId)
                    .addDatafiles(datafileIds),
                200
            )
        );
        assertEquals(
            ...
            testingClient.getSize(
                sessionId,
                new DataSelection().addDataset(datasetIds.get(2)),
                200
            )
        );
    }
    ...
    public void getSizesBad() throws Exception {
        testingClient.getSize(
            sessionId,
            new DataSelection().addDatafile(563L),
            404
        );
    }
    ...
    public void getSizesBad2() throws Exception {
        testingClient.getSize(
            sessionId,
            new DataSelection().addDatafile(563L).addDatafile(564L),
            404
        );
    }
    ...
    public void correctBehaviourInvestigations() throws Exception {
        try (
            InputStream stream = testingClient.getData(
                sessionId,
                new DataSelection()
                    .addInvestigations(Arrays.asList(investigationId)),
                Flag.NONE,
                0,
                200
            )
