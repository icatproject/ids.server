...
public class BaseTest {
    ...
    protected Path getDirOnFastStorage(Long dsId)
            throws IcatException_Exception {
        Dataset icatDs = (Dataset) icatWS.get(sessionId,
                "Dataset INCLUDE Investigation", dsId);
        return setup.getStorageDir()
                .resolve(Long.toString(icatDs.getInvestigation().getId()))
                .resolve(Long.toString(icatDs.getId()));
    }

    protected Path getFileOnArchiveStorage(Long dsId)
            throws IcatException_Exception {
        Dataset icatDs = (Dataset) icatWS.get(sessionId,
                "Dataset INCLUDE Investigation", dsId);
    ...
    protected void logTime(String msg) {
        ...
        if (msg != null) {
            if (time != 0) {
                System.out
                        .println(msg + " took " + (now - time) / 1000. + "s.");
            } else {
    ...
    protected void checkZipFile(Path file, List<Long> ids, int compressedSize)
            throws IOException {
    ...
    private void populateStorage(boolean twoLevel, String storageUnit,
        ...
        try {
            ...
            supportedDatafileFormat
                    .setId(icatWS.create(sessionId, supportedDatafileFormat));
            ...
            df1.setDataset(ds1);
            writeToFile(df1,
                    "df1 test content very compressible very compressible",
                    key);
            ...
            df2.setDataset(ds1);
            writeToFile(df2,
                    "df2 test content very compressible very compressible",
                    key);
            ...
            df3.setDataset(ds2);
            writeToFile(df3,
                    "df3 test content very compressible very compressible",
                    key);
            ...
            df4.setDataset(ds2);
            writeToFile(df4,
                    "df4 test content very compressible very compressible",
                    key);
            ...
            newFileLocation = setup.getUpdownDir()
                    .resolve("new_file_" + timestamp);
            ...
            System.err.println(
                    "Could not prepare ICAT db for testing: " + e.getMessage());
            e.printStackTrace();
            throw e;
        }
    ...
    protected void checkZipStream(InputStream stream, List<Long> datafileIdsIn,
            long compressedSize, int numLeft) throws IOException {
    ...
    protected void checkStream(InputStream stream, long id) throws IOException {
        ...
        for (Entry<String, Long> e : ids.entrySet()) {
            if (id == e.getValue()) {
                assertEquals(contents.get(e.getKey()),
                        new String(getOutput(stream)));
                found = true;
                break;
            }
        }
    ...
    protected void writeToFile(Datafile df, String content, String key)
        ...
        paths.put("ids/" + fac.getName() + "/" + inv.getName() + "/"
                + URLEncoder.encode(inv.getVisitId(), "UTF-8") + "/"
    ...
    private String digest(Long id, String location, String key)
    ...
    private void moveDatasetToArchive(String storageUnit, Dataset ds,
            String dsLoc, Facility fac, Investigation inv, String key)
            throws IOException, IcatException_Exception {
        ds = (Dataset) icatWS.get(sessionId, "Dataset INCLUDE Datafile",
                ds.getId());
        ...
        if (storageUnit.equals("DATASET")) {
            ...
            for (Datafile df : ds.getDatafiles()) {
                ...
                if (key == null) {
                    ...
                    file = top.resolve(getLocationFromDigest(df.getId(),
                            df.getLocation()));
                }
            ...
            for (Datafile df : ds.getDatafiles()) {
                ...
                if (key == null) {
                    ...
                    p = top.resolve(getLocationFromDigest(df.getId(),
                            df.getLocation()));
                }
        ...
        for (Datafile df : ds.getDatafiles()) {
            ...
            if (key == null) {
                ...
                file = top.resolve(
                        getLocationFromDigest(df.getId(), df.getLocation()));
            }
    ...
    protected void raceTest() throws Exception {
        ...
        Investigation inv = (Investigation) icatWS
                .search(sessionId, "Investigation").get(0);
        DatasetType dst = (DatasetType) icatWS.search(sessionId, "DatasetType")
                .get(0);
        ...
        for (int i = 0; i < 30; i++) {
            testingClient.put(sessionId, Files.newInputStream(path),
                    "uploaded_file" + i, dsid, supportedDatafileFormat.getId(),
                    "A rather splendid datafile", 201);
        }
        testingClient.archive(sessionId, new DataSelection().addDataset(dsid),
                204);
        ...
        testingClient.restore(sessionId, new DataSelection().addDataset(dsid),
                204);
        while (testingClient.getStatus(sessionId,
                new DataSelection().addDataset(dsid), 200) != Status.ONLINE) {
            Thread.sleep(1000);
        }
        ...
        testingClient.delete(sessionId, new DataSelection().addDataset(dsid),
                204);
    ...
    public void getDatafileIdsTest() throws Exception {
        ...
        ids = testingClient.getDatafileIds(sessionId,
                new DataSelection().addDatafile(datafileIds.get(0)), 200);
    ...
    public void bigDataSelectionTest() throws Exception {
        ...
        try (JsonReader parser = Json.createReader(new ByteArrayInputStream(
                restIcat.getProperties().getBytes()))) {
            assertEquals(
                    "maxEntities must have a fixed value in the icat.server for test to be useful",
                    20, parser.readObject().getInt("maxEntities"));
        }
        ...
        for (int i = 0; i < 45; i++) {
            Long dfid = testingClient.put(sessionId,
                    ...
                    "uploaded_file3_" + timestamp + i, datasetIds.get(0),
        ...
        idList2 = testingClient.getDatafileIds(sessionId,
                new DataSelection().addInvestigation(investigationId), 200);
    ...
    public void cloningTest() throws Exception {
        testingClient.restore(sessionId,
                new DataSelection().addDataset(datasetIds.get(0)), 204);
        waitForIds();
        assertEquals(104, testingClient.getSize(sessionId,
        ...
        Datafile df = (Datafile) icatWS.get(sessionId,
                "Datafile df INCLUDE df.dataset.investigation.facility", ndfid);
        Dataset ds = df.getDataset();
        Investigation inv = ds.getInvestigation();
        Facility fac = inv.getFacility();
        paths.put("ids/" + fac.getName() + "/" + inv.getName() + "/"
        ...
        crcs.put(df.getLocation(), df.getChecksum());
        fsizes.put(df.getLocation(), df.getFileSize());
        contents.put(df.getLocation(),
                "df1 test content very compressible very compressible");
        ids.put(df.getLocation(), df.getId());
        assertEquals(156, testingClient.getSize(sessionId,
                new DataSelection().addDataset(datasetIds.get(0)), 200));
        ...
        assertEquals("[3]", s.search(
                "SELECT COUNT(df) from Datafile df WHERE df.dataset.id = "
                        + dsid));
        testingClient.delete(sessionId,
                new DataSelection().addDatafile(datafileIds.get(0)), 204);
        assertEquals(2,
                getDirOnFastStorage(datasetIds.get(0)).toFile().list().length);
        assertEquals("[2]", s.search(
                "SELECT COUNT(df) from Datafile df WHERE df.dataset.id = "
                        + dsid));
        testingClient.delete(sessionId,
                new DataSelection().addDatafile(datafileIds.get(1)), 204);
        assertEquals(1,
                getDirOnFastStorage(datasetIds.get(0)).toFile().list().length);
        assertEquals("[1]", s.search(
                "SELECT COUNT(df) from Datafile df WHERE df.dataset.id = "
                        + dsid));
        testingClient.delete(sessionId, new DataSelection().addDatafile(ndfid),
                204);
        assertFalse(getDirOnFastStorage(datasetIds.get(0)).toFile().exists());
        assertEquals("[0]", s.search(
                "SELECT COUNT(df) from Datafile df WHERE df.dataset.id = "
                        + dsid));
    }
    ...
    public void reliabilityTest() throws Exception {
        ...
        Long dfid1 = testingClient.put(sessionId,
                Files.newInputStream(newFileLocation),
                "uploaded_file2_" + timestamp, datasetIds.get(0),
                supportedDatafileFormat.getId(), "A rather splendid datafile",
                201);

        testingClient.archive(sessionId, new DataSelection()
                .addDataset(datasetIds.get(0)).addDataset(datasetIds.get(1)),
                204);
        waitForIds();

        assertTrue(testingClient.getServiceStatus(sessionId, 200).getFailures()
                .isEmpty());

        setup.setReliability(0.);

        String preparedId = testingClient.prepareData(sessionId,
                new DataSelection().addDatafile(dfid1), Flag.NONE, 200);
        ...
        try {
            ...
            Set<Long> failures = testingClient.getServiceStatus(sessionId, 200)
                    .getFailures();
            assertEquals(1, failures.size());
            setup.setReliability(1.);
            testingClient.reset(preparedId, 204);
            assertTrue(testingClient.getServiceStatus(sessionId, 200)
                    .getFailures().isEmpty());
            while (!testingClient.isPrepared(preparedId, null)) {
                Thread.sleep(1000);
            }
        ...
        setup.setReliability(0.);
        Long dfid2 = testingClient.put(sessionId,
                Files.newInputStream(newFileLocation),
        ...
        setup.setReliability(1.);
        testingClient.restore(sessionId, new DataSelection()
                .addDataset(datasetIds.get(0)).addDatafile(dfid2), 204);
        waitForIds();
        System.out.println(testingClient.getStatus(sessionId,
                new DataSelection().addDataset(datasetIds.get(0))
                        .addDatafile(dfid2),
    ...
    protected void reliabilityTest2() throws Exception {
        ...
        setup.setReliability(0.);
        String preparedId = testingClient.prepareData(sessionId, dsel,
                Flag.NONE, 200);
    ...
    protected void reliabilityTest3() throws Exception {
        ...
        try {
            while (testingClient.getStatus(sessionId, dsel,
                    null) != Status.ONLINE) {
                Thread.sleep(1000);
            }
            fail("Should throw an error");
    ...
    protected void isPreparedTest() throws Exception {
        ...
        Investigation inv = (Investigation) icatWS
                .search(sessionId, "Investigation INCLUDE Facility").get(0);
        String invLoc = inv.getId() + "/";
        ...
        String preparedId = testingClient.prepareData(sessionId, dsel, Flag.ZIP,
                200);
