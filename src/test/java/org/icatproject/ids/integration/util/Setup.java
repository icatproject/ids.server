...
public class Setup {
    ...
    public Setup(String runPropertyFile) throws Exception {
        ...
        InputStream is =
            Setup.class.getClassLoader().getResourceAsStream("test.properties");
        try {
            testProps.load(is);
        } catch (Exception e) {
            System.err.println(
                "Problem loading test.properties: " +
                e.getClass() +
                " " +
                e.getMessage()
            );
        ...
        ShellCommand sc = new ShellCommand(
            "src/test/scripts/prepare_test.py",
        ...
        rootSessionId =
            TestUtils.login(icat, testProps.getProperty("login.root"));
        forbiddenSessionId =
            TestUtils.login(icat, testProps.getProperty("login.unauthorized"));
