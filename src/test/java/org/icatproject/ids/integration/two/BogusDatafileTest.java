...
public class BogusDatafileTest extends BaseTest {
    ...
    public void createBogusFiles() throws Exception {
        Dataset ds1 = (Dataset) icatWS.get(
            sessionId,
            "Dataset",
            datasetIds.get(0)
        );
        ...
        Dataset ds2 = (Dataset) icatWS.get(
            sessionId,
            "Dataset",
            datasetIds.get(1)
        );
        ...
        Dataset ds3 = (Dataset) icatWS.get(
            sessionId,
            "Dataset",
            datasetIds.get(2)
        );
     ...
     * dataset to archive storage, archive the dataset, and restore it.  Each step must deal gracefully with
    ...
    public void putWriteArchiveRestore() throws Exception {
        ...
        Datafile df1 = (Datafile) icatWS.get(
            sessionId,
            "Datafile",
            datafileIds.get(0)
        );
        locations.add(getLocationFromDigest(df1.getId(), df1.getLocation()));
        Datafile df2 = (Datafile) icatWS.get(
            sessionId,
            "Datafile",
            datafileIds.get(1)
        );
