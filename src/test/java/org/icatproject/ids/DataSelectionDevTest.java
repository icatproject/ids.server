...
public class DataSelectionDevTest {
    ...
    @Ignore(
        "Test requires a specific ICAT setup to produce meaningful results. See class javadoc comment."
    )
    @Test
    public void testCreateDataSelection() throws Exception {
        long startMs = System.currentTimeMillis();
        var dataSelectionFactory = DataSelectionServiceFactory.getInstanceOnlyForTesting(
            mockedPropertyHandler,
            icatReader
        );
        DataSelectionService dataSelectionService = dataSelectionFactory.getSelectionService(
            userSessionId,
            investigationIds,
            datasetIds,
            datafileIds,
