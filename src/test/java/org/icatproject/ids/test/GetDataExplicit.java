...
public class GetDataExplicit {
	...
	public void gettingDatafileRestoresItsDatasetTest() throws Exception {
		...
		File unrestoredDirOnFastStorage = new File(setup.getStorageDir(), unrestoredIcatDs.getLocation());
		File unrestoredZipOnFastStorage = new File(setup.getStorageZipDir(), unrestoredIcatDs.getLocation());
		try {
			testingClient.getDataTest(setup.getGoodSessionId(), null, null,
		...
		assertFalse("File " + unrestoredDirOnFastStorage.getAbsolutePath() + " shouldn't have been restored, but exist",
				unrestoredDirOnFastStorage.exists());
		assertFalse("Zip in " + unrestoredZipOnFastStorage.getAbsolutePath() + " shouldn't have been restored, but exist",
				unrestoredZipOnFastStorage.exists());
	}
	...
	public void gettingDatafileAndDatasetShouldRestoreBothDatasetsTest() throws Exception {
		...
		try {
			testingClient.getDataTest(setup.getGoodSessionId(), null, setup.getDatasetIds().get(DS_NUM_FROM_PROPS),
					setup.getDatafileIds().get(DF_NUM_FROM_PROPS), null, null, null, null);
		} catch (UniformInterfaceException e) {
			assertEquals(404, e.getResponse().getStatus());
			// Ignore exception. Check if the data will be restored.
		...
		assertTrue("File " + dirOnFastStorage0.getAbsolutePath() + " should have been restored, but doesn't exist",
				dirOnFastStorage0.exists());
		assertTrue("Zip in " + zipOnFastStorage0.getAbsolutePath() + " should have been restored, but doesn't exist",
				zipOnFastStorage0.exists());
