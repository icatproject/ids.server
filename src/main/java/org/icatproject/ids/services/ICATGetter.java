...
public class ICATGetter {
    ...
    private static String[] suffices = new String[] { "ICATService/ICAT?wsdl",
            "icat/ICAT?wsdl" };
     ...
     *                  non-empty file part it is used unchanged, otherwise
     *                  suffices are tried suitable for Glassfish and WildFly.
     * @return an ICAT
    ...
    public static ICAT getService(String urlString)
        ...
        if (emptyFile) {
            for (String suffix : suffices) {
                ...
                try {
                    icatService = new ICATService(new URL(icatUrlWsdl))
                            .getICATPort();
                    ...
                    if (cause != null
                            && cause.getMessage().contains("security")) {
