...
public class DataSelectionServiceFactory {
    ...
    private final static Logger logger = LoggerFactory
            .getLogger(DataSelectionServiceFactory.class);
    ...
    public static DataSelectionServiceFactory getInstance()
        ...
        if (instance == null) {
            ...
            instance = new DataSelectionServiceFactory(
    ...
    public static DataSelectionServiceFactory getInstanceOnlyForTesting(
            PropertyHandler propertyHandler, IcatReader reader)
            throws InternalException {
        if (instance == null) {
    ...
    public static DataSelectionService getService(String userSessionId,
    ...
    /**
     * created a DataSelectionBase object from already created data infos.
     * That's why no infos must be loaded from ICAT.
     * 
     * @param dsInfos
     * @param dfInfos
     * @param emptyDatasets
    ...
    public static DataSelectionService getService(
            ...
            long fileLength, RequestType requestType) throws InternalException {

        var prepared = new Prepared(dsInfos, dfInfos, emptyDatasets,
                fileLength);
        return DataSelectionServiceFactory.getService(prepared, requestType);
    }

    public static DataSelectionService getService(Prepared prepared,
        ...
        return DataSelectionServiceFactory.getInstance().createSelectionService(
                prepared.dsInfos, prepared.dfInfos, prepared.emptyDatasets,
                invIds, dsids, dfids, prepared.fileLength, prepared.zip,
                prepared.compress, requestType);
    }

    private DataSelectionServiceFactory(PropertyHandler propertyHandler,
            IcatReader reader) throws InternalException {
    ...
    private DataSelectionService createSelectionService(
            SortedMap<Long, DataInfoBase> dsInfos,
            SortedMap<Long, DataInfoBase> dfInfos, Set<Long> emptyDatasets,
            List<Long> invids, List<Long> dsids, List<Long> dfids, long length,
        ...
        if (storageUnit == null)
            return new DataSelectionServiceForSingleLevelStorage(dsInfos,
                    dfInfos, emptyDatasets, invids, dsids, dfids, length, zip,
                    compress, requestType);

        else if (storageUnit == StorageUnit.DATAFILE)
            return new DataSelectionServiceForStorageUnitDatafile(dsInfos,
                    dfInfos, emptyDatasets, invids, dsids, dfids, length, zip,
                    compress, requestType);

        else if (storageUnit == StorageUnit.DATASET)
            return new DataSelectionServiceForStorageUnitDataset(dsInfos,
                    dfInfos, emptyDatasets, invids, dsids, dfids, length, zip,
                    compress, requestType);

        else
            throw new InternalException("StorageUnit " + storageUnit
                    + " unknown. Maybe you forgot to handle a new StorageUnit here?");

    }

    public DataSelectionService getSelectionService(String userSessionId,
            String investigationIds, String datasetIds, String datafileIds,
            RequestType requestType)
            throws InternalException, BadRequestException, NotFoundException,
            InsufficientPrivilegesException, NotImplementedException {
        ...
        boolean dfWanted = returns == Returns.DATASETS_AND_DATAFILES
                || returns == Returns.DATAFILES;
        boolean dsWanted = returns == Returns.DATASETS_AND_DATAFILES
        ...
        try {
            logger.debug("useReaderForPerformance = {}",
                    propertyHandler.getUseReaderForPerformance());
            if (propertyHandler.getUseReaderForPerformance()) {
                // if this is set, use a REST session for the reader account
                // where possible
                // to improve performance due to the final database queries
                // being simpler
                restSessionToUse = restIcat
                        .getSession(this.icatReader.getSessionId());
            }
        ...
        return prepareFromIds(dfWanted, dsWanted, dfids, dsids, invids,
                userSessionId, restSessionToUse, userRestSession, requestType);
    }

    private DataSelectionService prepareFromIds(boolean dfWanted,
            boolean dsWanted, List<Long> dfids, List<Long> dsids,
            List<Long> invids, String userSessionId, Session restSessionToUse,
            Session userRestSession, RequestType requestType)
        ...
        try {
            ...
            for (Long dfid : dfids) {
                ...
                if (dss.size() == 1) {
                    ...
                    if (dfWanted) {
                        Datafile df = (Datafile) icat.get(userSessionId,
                                "Datafile", dfid);
                        length += df.getFileSize();
                        String location = LocationHelper.getLocation(dfid,
                                df.getLocation());
                        dfInfos.put(df.getId(),
                                new DatafileInfo(dfid, df.getName(), location,
                                        df.getCreateId(), df.getModId(), dsid));
                    }
                } else {
            ...
            for (Long dsid : dsids) {
                Dataset ds = (Dataset) icat.get(userSessionId,
                        "Dataset ds INCLUDE ds.investigation.facility", dsid);
                dsInfos.put(dsid, new DatasetInfo(ds));
                // dataset access for the user has been checked so the REST
                // session for the
                // reader account can be used if the IDS setting to allow this
                // is enabled
            ...
            for (Long invid : invids) {
                ...
                JsonArray result = Json
                        ...
                        .readArray().getJsonArray(0);
                manyDss(dsInfos, emptyDatasets, dfInfos, invid, dfWanted,
                        userRestSession, restSessionToUse, result);
            ...
            org.icatproject.icat.client.IcatException.IcatExceptionType type = e
                    .getType();
        ...
        return this.createSelectionService(dsInfos, dfInfos, emptyDatasets,
                invids, dsids, dfids, length, false, false, requestType);
    }
        ...
        if (idList == null) {
            ...
            for (String id : ids) {
                try {
                    ...
                    throw new BadRequestException("The " + thing
                            + " parameter '" + idList + "' is not a valid "
    ...
    private void manyDfs(Map<Long, DataInfoBase> dfInfos, long dsid,
            Session restSessionToUse, JsonArray result) throws IcatException,
            InsufficientPrivilegesException, InternalException {
        // dataset access for the user has been checked so the REST session for
        // the
        // reader account can be used if the IDS setting to allow this is
        // enabled
        ...
        if (count != 0) {
            if (count <= maxEntities) {
                ...
                for (JsonValue tupV : result) {
                    ...
                    dfInfos.put(dfid,
                            new DatafileInfo(dfid, tup.getString(1), location,
                                    tup.getString(3), tup.getString(4), dsid));
                }
            } else {
                long half = (min + max) / 2;
                String query = "SELECT min(df.id), max(df.id), count(df.id) FROM Datafile df WHERE df.dataset.id = "
    ...
    private void manyDss(Map<Long, DataInfoBase> dsInfos,
            HashSet<Long> emptyDatasets, Map<Long, DataInfoBase> dfInfos,
            Long invid, boolean dfWanted, Session userRestSession,
            Session restSessionToUseForDfs, JsonArray result)
            throws IcatException, InsufficientPrivilegesException,
            InternalException {
        long min = result.getJsonNumber(0).longValueExact();
        long max = result.getJsonNumber(1).longValueExact();
        long count = result.getJsonNumber(2).longValueExact();
        ...
        if (count != 0) {
            if (count <= maxEntities) {
                ...
                query = "SELECT ds.id, ds.name, ds.location FROM Dataset ds WHERE ds.investigation.id = "
                        + invid + " AND ds.id BETWEEN " + min + " AND " + max;
                result = Json
                        .createReader(new ByteArrayInputStream(
                                userRestSession.search(query).getBytes()))
                        .readArray();
                for (JsonValue tupV : result) {
                    ...
                    dsInfos.put(dsid,
                            new DatasetInfo(dsid, tup.getString(1),
                                    tup.getString(2, null), invid, invName,
    ...
    private void createRequestTypeToReturnsMapping() throws InternalException {
        ...
        this.requestTypeToReturnsMapping.put(RequestType.DELETE,
                Returns.DATASETS_AND_DATAFILES);
        this.requestTypeToReturnsMapping.put(RequestType.GETDATAFILEIDS,
                Returns.DATAFILES);
        this.requestTypeToReturnsMapping.put(RequestType.GETSIZE,
                Returns.DATASETS_AND_DATAFILES);
        this.requestTypeToReturnsMapping.put(RequestType.PREPAREDATA,
                Returns.DATASETS_AND_DATAFILES);
        this.requestTypeToReturnsMapping.put(RequestType.RESET,
                Returns.DATASETS_AND_DATAFILES);
        this.requestTypeToReturnsMapping.put(RequestType.WRITE,
                Returns.DATASETS_AND_DATAFILES);
        this.requestTypeToReturnsMapping.put(RequestType.GETDATA,
                Returns.DATASETS_AND_DATAFILES);

        if (storageUnit == null) {
            this.requestTypeToReturnsMapping.put(RequestType.GETSTATUS,
                    Returns.DATASETS);
        } else if (storageUnit == StorageUnit.DATAFILE) {
            this.requestTypeToReturnsMapping.put(RequestType.GETSTATUS,
                    Returns.DATAFILES);
            this.requestTypeToReturnsMapping.put(RequestType.RESTORE,
                    Returns.DATAFILES);
            this.requestTypeToReturnsMapping.put(RequestType.ARCHIVE,
                    Returns.DATAFILES);
        } else if (storageUnit == StorageUnit.DATASET) {
            this.requestTypeToReturnsMapping.put(RequestType.GETSTATUS,
                    Returns.DATASETS);
    ...
    private Returns getReturns(RequestType requestType)
        ...
        // is this needed here?
        // if(this.propertyHandler.getStorageUnit() == null) throw new
        // NotImplementedException("This operation is unavailable for single
        // level storage");

        throw new NotImplementedException("There is to mapping for RequestType."
                + requestType + " and StorageUnit."
