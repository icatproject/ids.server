...
public class GetSizeHandlerForFastProcessing extends RequestHandlerBase {
    ...
    public GetSizeHandlerForFastProcessing(
        String ip,
        String sessionId,
    ...
    public ValueContainer handleRequest()
        ...
        List<Long> dfids = DataSelectionService.getValidIds(
            "datafileIds",
            dataController.datafileIds
        );
        List<Long> dsids = DataSelectionService.getValidIds(
            "datasetIds",
            dataController.datasetIds
        );
        List<Long> invids = DataSelectionService.getValidIds(
            "investigationIds",
        ...
        if (dfids.size() + dsids.size() + invids.size() == 1) {
            ...
            if (size == 0) {
                try {
                    if (dfids.size() != 0) {
                        Datafile datafile = (Datafile) serviceProvider
                            .getIcat()
                            .get(
                                dataController.getOperationId(),
                                "Datafile",
    ...
    private long evalSizeFor(String sessionId, String where, StringBuilder sb)
        throws InternalException {
        String query =
            "SELECT SUM(df.fileSize) from Datafile df WHERE " +
            where +
            " IN (" +
            sb.toString() +
