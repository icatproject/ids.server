...
public class PutHandler extends RequestHandlerBase {
    ...
    public PutHandler(
        ...
        try {
            ...
            throw new RuntimeException(
                "PutHandler reports " + e.getClass() + " " + e.getMessage()
            );
        }
    }
    ...
    public ValueContainer handleRequest()
        ...
        try {
            ...
            DataControllerBase.validateUUID(
            ...
            if (datafileCreateTimeString != null) {
                try {
                    datafileCreateTime =
                        Long.parseLong(datafileCreateTimeString);
                } catch (NumberFormatException e) {
                    throw new BadRequestException(
            ...
            try {
                ds =
                    (Dataset) serviceProvider
                        ...
                        .get(
                            sessionId,
                            "Dataset INCLUDE Investigation, Facility",
                            datasetId
                        );
            ...
            try (
                Lock lock = serviceProvider
                    .getLockManager()
                    .lock(dsInfo, LockType.SHARED)
            ) {
                if (storageUnit == StorageUnit.DATASET) {
                    var dfInfos = new TreeMap<Long, DataInfoBase>();
                    Set<Long> emptyDatasets = new HashSet<>();
                    try {
                        List<Object> counts = serviceProvider
                            .getIcat()
                            .search(
                                sessionId,
                                "COUNT(Datafile) <-> Dataset [id=" +
                                dsInfo.getId() +
                                "]"
                            );
                    ...
                    DataSelectionService dataSelectionService = DataSelectionServiceFactory.getService(
                        dsInfos,
                        dfInfos,
                        emptyDatasets,
                        0,
                ...
                try {
                    location =
                        serviceProvider.getMainStorage().put(dsInfo, name, is);
                ...
                try {
                    ...
                    try {
                        userId =
                            serviceProvider.getIcat().getUserName(sessionId);
                    } catch (IcatException_Exception e1) {
                        logger.error(
                            "Unable to get user name for session " +
                            sessionId +
                            " so mainStorage.delete of " +
                            location +
                            " may fail"
                        );
                    }
            ...
            if (wrap) {
                ...
                return new ValueContainer(
                    Response
                        ...
                        .entity(
                            pre + baos.toString().replace("'", "\\'") + suffix
                        )
                        .build()
                );
            } else {
    ...
    private Long registerDatafile(
        ...
        String description,
        String doi,
        Long datafileCreateTime,
        Long datafileModTime
    )
        ...
        try {
            ...
            if (key != null) {
                df.setLocation(
                    location + " " + IcatSecurity.digest(dfId, location, key)
                );
                icat.update(sessionId, df);
            }

            logger.debug(
                "Registered datafile for dataset {} for {}",
                dataset.getId(),
                name + " at " + location
            );
            return dfId;
