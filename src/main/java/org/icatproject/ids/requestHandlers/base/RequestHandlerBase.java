...
/**
 * This base class represents all common properties and methods which are needed
 * by each request handler. Request handlers schould be added to the internal
 * request handler list in RequestHandlerService, to be able to be called.
 */
public abstract class RequestHandlerBase {
    ...
    /**
     * The core method of each request handler. It has to be overwritten in the
     * concrete implementation to provide an individual request handling
     * 
    ...
    private void transmit(long start) throws BadRequestException {
        if (ServiceProvider.getInstance().getLogSet()
                .contains(this.getCallType())) {
            try {
                String body = this.provideTransmissionBody();
                ServiceProvider.getInstance().getTransmitter()
                        .processMessage("archive", ip, body, start);
            } catch (IcatException_Exception e) {
                logger.error("Failed to prepare jms message " + e.getClass()
                        + " " + e.getMessage());
            }
        }
    }
    ...
    /**
     * Override to add additional parameters to the log output for the current
     * request
     * 
     * @return
     */
    ...
    /**
     * each handler should provide its own CallType which is needed to create
     * the Transmitter message
     * 
