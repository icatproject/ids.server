...
public abstract class DataControllerBase {
    ...
    public abstract boolean mustZip(
        boolean zip,
    ...
    public static void validateUUID(String thing, String id)
        throws BadRequestException {
        if (
            id == null || !uuidRegExp.matcher(id).matches()
        ) throw new BadRequestException(
            "The " + thing + " parameter '" + id + "' is not a valid UUID"
        );
    }
    ...
    protected String createSessionId() throws InternalException {
        ...
        try {
            sessionId =
                ServiceProvider.getInstance().getIcatReader().getSessionId();
